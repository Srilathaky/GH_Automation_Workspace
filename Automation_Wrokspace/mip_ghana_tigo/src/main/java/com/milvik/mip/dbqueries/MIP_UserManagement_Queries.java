	...
	public static Map<String, String> getUserInfo(String mobilenum) {
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT user_uid,fname,sname,dob,gender,email_id,role_name,branch_name FROM user_details ud join role_master rm on ud.role_id=rm.role_id join branch_details bd on ud.branch_id=bd.branch_id where ud.msisdn='"
							+ mobilenum + "';");
			while (result.next()) {
