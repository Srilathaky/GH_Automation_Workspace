...
public class MIP_RoleManagement_Queries {
	...
	public static List<String> getRoleAcess(String rolename) {
		...
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT m.menu_name FROM menu m JOIN menu_role_mapping mp on m.menu_id=mp.menu_id where mp.role_id=("
							+ "SELECT role_id FROM role_master where role_name='"
							+ rolename + "') ;");
			while (result.next()) {
				roleaccess.add(result.getString("menu_name"));
			}
	...
	public static List<String> getMappedRoles(String roleName) {
		...
		try {
			...
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT me.menu_name FROM menu me join menu_role_mapping mr on mr.menu_id=me.menu_id where mr.role_id=(select role_id from role_master where role_name='"
							+ roleName + "');");
			while (result.next()) {
				rolemap.add(result.getString("menu_name"));
			}
