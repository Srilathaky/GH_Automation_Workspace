	...
	public static Map<String, String> getAdminConfigInfo() {
		try {
			...
			adminConfigdetails.put("announcement_message",
					result.getString("data_val"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT data_val FROM admin_config where param='commission_percentage';");
			result.next();
			adminConfigdetails.put("commission_percentage",
					result.getString("data_val"));
			result = MIP_DataBaseConnection.st
					.executeQuery("SELECT data_val FROM admin_config where param='offer_subscription_last_day';");
			result.next();
			adminConfigdetails.put("offer_subscription_last_day",
					result.getString("data_val"));
