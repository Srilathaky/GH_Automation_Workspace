	...
	public void clickOnUser(String mobileNumber) {
		...
		for (int i = 0; i < this.getNumberofUser() / 10; i++) {
			this.waitForElementToVisible(By
					.xpath("//table[@id='userList']/tbody"));
			List<WebElement> mobilenum = driver.findElements(By
					.xpath("//table[@id='userList']/tbody/tr/td[3]"));
			...
			if (flag == 0) {
				try {
					...
					logger.info("User with mobilenumber " + mobileNumber
