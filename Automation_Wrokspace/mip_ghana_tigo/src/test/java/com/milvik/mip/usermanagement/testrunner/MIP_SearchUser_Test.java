...
public class MIP_SearchUser_Test {
	...
	public void searchUserOne() throws Throwable {
		try {
			...
			Assert.assertTrue(searchuser.userID.isDisplayed());
		} catch (Throwable t) {
	...
	public void searchUserTwo(String testcase, String fname, String sname,
			String msisdn, String userid, String role) throws Throwable {
		try {
			...
			searchuser = PageFactory.initElements(
					MIP_Test_Configuration.driver, MIP_SearchUserPage.class);
			searchuser.enterSearchCriteria(fname, sname, msisdn, userid, role)
					.clickOnSearch();
			boolean val1 = false;
			boolean val2 = false;
			...
			if (!userid.equals("")) {
				List<WebElement> userlist = searchuser.getUSerId();
				for (int i = 0; i < userlist.size(); i++) {
					Assert.assertTrue(userlist
							.get(i)
							.getText()
							.contains(
									MIP_AdminConfig_Queries.getLoginPrefix()
											.trim() + userid.trim()));
				}
			}
		} catch (Throwable t) {
	...
	public void searchUserThree(String fname, String sname, String msisdn,
		...
		try {
			...
			Assert.assertTrue(searchuser.userID.getText().equals(""));
		} catch (Throwable t) {
